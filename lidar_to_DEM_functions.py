    ...
    try:
        for f in files_in_direct:
            if f[-4:] == ".laz":  # Convert laz files to LAS files
                cmd("%slaszip.exe -i %s\\%s -o %s\\%s_noprj.las" % (lasbin, lidardir, f, laspath, f[:-4]))
                print("%slas2las.exe -i %s\\%s_noprj.las -o %s\\%s.las RUNNING..." % (lasbin, laspath, f[:-4], laspath, f[:-4]))
                cmd("%slas2las.exe -i %s\\%s_noprj.las -o %s\\%s.las" % (lasbin, laspath, f[:-4], laspath, f[:-4]))
        ...
        raw_las_dataset = arcpy.CreateLasDataset_management(laspath, lidardir + "\\raw_las_dataset.lasd", spatial_reference=spatial_ref, compute_stats=True)
        print("Un-projected LAS Dataset made @ %s" % raw_las_dataset)

        lidar_footprint = arcpy.PointFileInformation_3d(raw_las_dataset, lidardir + "\\las_footprint_pre_dissolve", "LAS", input_coordinate_system=spatial_ref)
...
def define_ground_polygon(lidar_footprint, lidardir, spatialref_shp, naipdir, ndvi_thresh, aoi_shp):
    ...
    if len(naip_imagery) > 1:
        ...
        naip_imagery = arcpy.MosaicToNewRaster_management(add_to_mosaic, output_location=lidardir, raster_dataset_name_with_extension="NAIP_mos.tif", number_of_bands=4)
    else:
    ...
    try:
        ...
        if aoi_shp != '':
            ground_poly = arcpy.Erase_analysis(spatial_extent, veg_poly, temp_files + "//ground_poly_full.shp")
            centerline_buff_prj = arcpy.Project_management(centerline_buff, temp_files + "//aoi_prj_to_inref.shp", out_coor_system=spatial_ref)
            ground_poly = arcpy.Clip_analysis(ground_poly, centerline_buff, lidardir + "//ground_poly.shp")

        else:
            ground_poly = arcpy.Erase_analysis(spatial_extent, veg_poly, lidardir + "//ground_poly.shp")
